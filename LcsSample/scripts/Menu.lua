function BuildMenu(context)
	...
	function GameMenu:GetSettings()
		...
		for n=0,count-1 do
			...
			if entity:GetClass()==Object.CameraClass then
				...
				local aa = tostring(camera:GetMultisampleMode()).."x"
	...
	function GameMenu:ApplySettings()
		...
		--Antialias
		local aa = self.antialias:GetItemText(self.antialias:GetSelectedItem())
		aa = string.gsub(aa,"x","")
		...
		local gfxmode = self.screenres:GetItemText(self.screenres:GetSelectedItem())
		gfxmode = self:splitstring(gfxmode,"x")
		local window = self.context:GetWindow()
		window:SetLayout(0,0,gfxmode[1],gfxmode[2])
		
		VSyncMode = self.vsync:GetState()
		
		world:SetLightQuality(self.lightquality:GetSelectedItem())
		world:SetWaterQuality(self.waterquality:GetSelectedItem())
		Texture:SetDetail(self.texturequality:CountItems()-1-self.texturequality:GetSelectedItem())
		Texture:SetAnisotropy(self.afilter:GetSelectedItem())
		Texture:SetTrilinearFilterMode(self.tfilter:GetState())
		world:SetTerrainQuality(self.terrainquality:GetSelectedItem())
