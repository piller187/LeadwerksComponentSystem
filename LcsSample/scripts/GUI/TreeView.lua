...
Script.itemindent = 14
Script.sliderwidth = 20
Script.textposition = 18
...
function Script:Draw()
	...
	--Draw nodes
	local n,img,text,node,indent
	for n=1,#self.visiblenodes do
		node = self.visiblenodes[n].widget
		indent = self.visiblenodes[n].indent
		text = node:GetText()
		
		if self.selectednodes[node] then
			gui:SetColor(51/255/2,151/255/2,1/2)
			gui:DrawRect(-self.slider[1]:GetSliderValue() - 4*scale + self.textposition*scale + pos.x + 2*scale + indent*scale, -self.slider[0]:GetSliderValue() + pos.y + 2*scale + scale*self.itemheight*(n-1) - 2*scale,gui:GetTextWidth(text)+4*scale,(self.itemheight - self.itemspacing + 4) * scale)
		end
		
		gui:SetColor(self.color.foreground.r,self.color.foreground.g,self.color.foreground.b,self.color.foreground.a)
		gui:DrawText(text,-self.slider[1]:GetSliderValue() + pos.x + self.textposition * scale + indent * scale, -self.slider[0]:GetSliderValue() + pos.y + 2*scale + scale*self.itemheight*(n-1),gui:GetTextWidth(text),(self.itemheight - self.itemspacing) * scale)
		
		if node:CountChildren()>0 then
			if node:Collapsed() then
				img = self.image.expand
			else
				img = self.image.collapse
			end
			gui:DrawImage(img,-self.slider[1]:GetSliderValue() + pos.x + 4*scale + indent, -self.slider[0]:GetSliderValue() + pos.y + 4*scale + scale*self.itemheight*(n-1))
	...
	if button==Mouse.Left then
		...
		if nodeindex~=nil then
			if x < (self.textposition + self.visiblenodes[nodeindex].indent) * scale then
				...
				if nodeindex~=nil then
					local node = self.visiblenodes[nodeindex].widget
					if self.ctrlpressed==true then
						if self.selectednodes[node]~=true then
							self:ScrollToNode(nodeindex)
						end
						self.selectednodes[node]=not self.selectednodes[node]
						if self.selectednodes[node] then
						...
						if self.shiftpressed==true and self.lasttouchednode~=nil then
							...
							for n=self.lasttouchednode,nodeindex,iterator do
								if self.selectednodes[self.visiblenodes[n].widget]~=true then
									self.selectednodes[self.visiblenodes[n].widget]=true
	...
	if key==Key.Down then
		if self.lasttouchednode~=nil then
			if self.lasttouchednode<#self.visiblenodes then
				...
				self.selectednodes[self.visiblenodes[self.lasttouchednode + 1].widget] = true
...
function Script:DoubleClick(button,x,y)
	...
	if button==Mouse.Left then
		local scale = self.widget:GetGUI():GetScale()
		if self.visiblenodes==nil then self:UpdateNodes(self.widget) end
		local nodeindex = self:GetNodeAtPosition(y)
		if nodeindex~=nil then
			if x >= (self.textposition + self.visiblenodes[nodeindex].indent) * scale then
