...
function Script:Start()
	...
	--Vertical slider
	self.slider[0] = Widget:Create(sz.width-self.sliderwidth*scale,0,self.sliderwidth,sz.height--[[-self.sliderwidth]],self.widget)
...
function Script:AddText(text)
	local morelines = text:split("\r")
	local n
	for n=1,#morelines do
		self.lines[#self.lines+1] = morelines[n]
		self.linewidth[#self.lines] = self.widget:GetGUI():GetTextWidth(morelines[n])
...
function Script:Draw(x,y,width,height)
	...
	if self.sellen~=0 then		
		...
		for line=math.max(firstline,firstvisibleline),math.min(lastvisibleline,lastline) do
			...
			gui:DrawRect(pos.x + px + self.slider[1]:GetSliderValue()*-1, self.slider[0]:GetSliderValue()*-1+pos.y + y+(lh-caretheight)/2,w,caretheight,0)
		end
	end
	...
	if self.cursorblinkmode then
		if self.focused then
			...
			local y = (self.currentline-1) * lineheight
			gui:DrawLine(pos.x + x + self.slider[1]:GetSliderValue()*-1,self.slider[0]:GetSliderValue()*-1+pos.y+y + (lh-caretheight)/2,pos.x + x + self.slider[1]:GetSliderValue()*-1,self.slider[0]:GetSliderValue()*-1+pos.y+y + (lh+caretheight)/2)
		end
	end
