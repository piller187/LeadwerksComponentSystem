...
void TransparencyFilter()
{
	int ix = int(mod(float(int(gl_FragCoord.x+0.5)),2.0f)+0.5);
	int iy = int(mod(float(int(gl_FragCoord.y+0.5)),2.0f)+0.5);
...
//	texcoord.y = -texcoord.y;
	...
	vec4 screencolor = texture(texture10,texcoord + ex_normal.xy*0.1);
	outcolor = screencolor * 0.5 + outcolor * 0.5;
